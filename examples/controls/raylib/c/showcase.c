/* Host-owned raylib window, input and frame loop. */
...
int main(void) {
    int result = 1;
    NuiRaylibRenderer *renderer = NULL;
    NuiUi *ui = nui_create(HTML, CSS, 940, 640);
    if (!ui) goto error;
    SetConfigFlags(FLAG_WINDOW_RESIZABLE | FLAG_WINDOW_HIGHDPI);
    InitWindow(940, 640, "Controls — C / raylib");
    if (!IsWindowReady()) goto error;
    SetTargetFPS(60);
    SetExitKey(KEY_NULL); // Let Escape dismiss UI popups; closing is host policy.
    renderer = nui_raylib_create();
    ...
    while (!WindowShouldClose()) {
        ...
        if (width > 0 && height > 0 && !nui_set_viewport(ui,(float)width,(float)height)) goto error;
        if (!nui_raylib_input(ui)) goto error; // Key/mouse snapshot; game can read it too.
        // The host owns the character queue and can also route text to its game.
        for (int ch = GetCharPressed(); ch; ch = GetCharPressed()) {
            if (IsWindowFocused() && !nui_raylib_text(ui,ch)) goto error;
        }
        if (!update(ui)) goto error;
        BeginDrawing();
        ClearBackground((Color){243,243,243,255});
        // Draw your game here. The UI does not begin or end your frame.
        int drawn = nui_renderer_render(renderer,ui,draw,NULL);
        EndDrawing();
        if (!drawn || !nui_end_frame(ui)) goto error;
    }
    result = 0;
    goto cleanup;
error:
    fprintf(stderr, "%s\n", nui_last_error());
cleanup:
    nui_raylib_destroy(renderer); // Release UI textures before closing raylib.
    nui_destroy(ui);
