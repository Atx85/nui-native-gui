...
//! Controls, events and application updates using the SDK-owned raylib connector.
...
fn draw(_: ?*anyopaque, renderer: ?*anyopaque, _: c.NuiString, _: c.NuiRect, content: c.NuiRect, _: c.NuiRect) callconv(.c) i32 {
    ...
    return if (c.nui_raylib_fill(renderer, square, .{ .r = 54, .g = 95, .b = 221, .a = 255 }) == 1) 0 else 1;
}
...
pub fn main() !void {
    ...
    const window = c.nui_raylib_open("Controls — Zig / raylib", 940, 640, @intFromBool(smoke)) orelse return error.Window;
    defer _ = c.nui_raylib_close(window);
    ...
    try app.ok(c.nui_raylib_register_image(window, "first", 2, 2, &first, first.len));
    try app.ok(c.nui_raylib_register_image(window, "second", 2, 2, &second, second.len));
    var alternate = false;
    var frame: usize = 0;
    while (!smoke or frame < 8) : (frame += 1) {
        const event = c.nui_raylib_poll(window, ui);
        if (event < 0) return error.Poll;
        if (event == 1) break;
        try update(ui, &alternate);
        try app.ok(c.nui_raylib_clear(window, .{ .r = 243, .g = 243, .b = 243, .a = 255 }));
        try app.ok(c.nui_raylib_render(window, ui, draw, null));
        try app.ok(c.nui_raylib_present(window));
