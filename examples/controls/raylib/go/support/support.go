...
static inline int32_t render(NuiRaylib *w,NuiUi *ui,int canvas) {
	return nui_raylib_render(w,ui,canvas ? exampleDraw : NULL,NULL);
}
*/
import "C"
import (
...
type Window struct{ p *C.NuiRaylib }
...
func (w *Window) Image(id string, width, height uint32, pixels []byte) {
	...
	p, f := c(id)
	defer f()
	ok(C.nui_raylib_register_image(w.p, p, C.uint32_t(width), C.uint32_t(height), (*C.uint8_t)(unsafe.Pointer(&pixels[0])), C.size_t(len(pixels))))
}
func (r *Renderer) Fill(b Rect, color Color) {
	ok(C.nui_raylib_fill(r.p, C.NuiRect{x: C.float(b.X), y: C.float(b.Y), w: C.float(b.W), h: C.float(b.H)}, C.NuiColor{r: C.uint8_t(color.R), g: C.uint8_t(color.G), b: C.uint8_t(color.B), a: C.uint8_t(color.A)}))
}
...
func OpenWindow(name string, width, height int, smoke bool) *Window {
	...
	window := &Window{C.nui_raylib_open(title, C.uint32_t(width), C.uint32_t(height), hidden)}
	Require(window.p != nil)
	return window
}
func (w *Window) Close() { ok(C.nui_raylib_close(w.p)) }
func (w *Window) Poll(ui *Ui) bool {
	event := C.nui_raylib_poll(w.p, ui.p)
	Require(event >= 0)
	return event == 0
}
func (w *Window) Clear(color Color) {
	ok(C.nui_raylib_clear(w.p, C.NuiColor{r: C.uint8_t(color.R), g: C.uint8_t(color.G), b: C.uint8_t(color.B), a: C.uint8_t(color.A)}))
}
...
func (w *Window) Present() { ok(C.nui_raylib_present(w.p)) }
