/* Host-owned SDL3 window, events, rendering and presentation. */
...
static int32_t draw(void *data, void *painter, NuiString id, NuiRect bounds,
    ...
    return nui_renderer_fill(painter, square, blue) == 1 ? 0 : 1;
}

int main(void) {
    int result = 1;
    SDL_Window *window = NULL;
    SDL_Renderer *host = NULL;
    NuiSdlRenderer *renderer = NULL;
    NuiUi *ui = nui_create(HTML, CSS, 940, 640);
    if (!ui || !SDL_Init(SDL_INIT_VIDEO)) goto error;
    if (!SDL_CreateWindowAndRenderer("Controls — C / SDL3", 940, 640,
            SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIGH_PIXEL_DENSITY, &window, &host)) goto error;
    SDL_SetRenderVSync(host, 1);
    renderer = nui_sdl_create(host);
    ...
    if (!nui_renderer_register_image(renderer, "first", 2, 2, first, sizeof first) ||
        !nui_renderer_register_image(renderer, "second", 2, 2, second, sizeof second)) goto error;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (!nui_sdl_event(renderer, ui, &event)) goto error;
        }
        if (!running) break;
        int width, height;
        if (!SDL_GetWindowSize(window, &width, &height)) goto error;
        if (width <= 0 || height <= 0) continue;
        if (!SDL_SetRenderLogicalPresentation(host,width,height,SDL_LOGICAL_PRESENTATION_STRETCH)) goto error;
        ...
        SDL_SetRenderDrawColor(host, 243, 243, 243, 255);
        if (!SDL_RenderClear(host)) goto error;
        // Draw your game here. The UI only borrows its renderer.
        if (!nui_renderer_render(renderer, ui, draw, NULL)) goto error;
        if (!SDL_RenderPresent(host)) goto error;
...
cleanup:
    nui_sdl_destroy(renderer); // Release UI textures while the host is alive.
    nui_destroy(ui);
    SDL_DestroyRenderer(host);
